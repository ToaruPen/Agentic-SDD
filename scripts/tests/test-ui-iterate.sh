...
script_src="$repo_root/scripts/shell/ui-iterate.sh"
...
mkdir -p "$tmpdir/scripts/shell"
cp -p "$script_src" "$tmpdir/scripts/shell/ui-iterate.sh"
chmod +x "$tmpdir/scripts/shell/ui-iterate.sh"
...
(cd "$tmpdir" && ./scripts/shell/ui-iterate.sh 99 --dry-run --route /kiosk >"$dry_out" 2>&1)
...
(cd "$tmpdir" && ./scripts/shell/ui-iterate.sh 99 round-01 --route /kiosk --skip-checks --capture-cmd "$cap_cmd") >/dev/null
...
(cd "$tmpdir" && ./scripts/shell/ui-iterate.sh 99 --route /kiosk --skip-checks --capture-cmd "$cap_cmd") >/dev/null
...
(cd "$tmpdir" && ./scripts/shell/ui-iterate.sh 99 round-03 --route /staff --skip-capture \
...
set +e
(cd "$tmpdir" && ./scripts/shell/ui-iterate.sh 99 round-04 --route /staff --skip-capture --check-cmd "exit 7") >/dev/null 2>"$tmpdir/fail-stderr"
code=$?
set -e
...
set +e
(cd "$tmpdir" && ./scripts/shell/ui-iterate.sh 99 round-05 --route /kiosk --skip-capture) >/dev/null 2>"$tmpdir/no-checks-stderr"
code_no_checks=$?
set -e
...
set +e
(cd "$tmpdir" && PATH="$tmpdir/bin:$PATH" UI_ITERATE_NODE_SCENARIO=ok UI_ITERATE_CURL_SCENARIO=ok ./scripts/shell/ui-iterate.sh 99 round-06 --route /kiosk --skip-checks) >/dev/null 2>"$tmpdir/builtin-success-stderr"
code=$?
set -e
...
set +e
(cd "$tmpdir" && ./scripts/shell/ui-iterate.sh 99 round-07 --route /kiosk --skip-checks --desktop-size 10x10) >/dev/null 2>"$tmpdir/invalid-size-stderr"
code_invalid_dim=$?
set -e
...
set +e
(cd "$tmpdir" && PATH="$tmpdir/bin:$PATH" UI_ITERATE_NODE_SCENARIO=toint UI_ITERATE_CURL_SCENARIO=ok ./scripts/shell/ui-iterate.sh 99 round-08 --route /kiosk --skip-checks) >/dev/null 2>"$tmpdir/toint-stderr"
code_toint=$?
set -e
...
set +e
(cd "$tmpdir" && PATH="$tmpdir/bin:$PATH" UI_ITERATE_NODE_SCENARIO=nav-fail UI_ITERATE_CURL_SCENARIO=ok ./scripts/shell/ui-iterate.sh 99 round-09 --route /kiosk --skip-checks) >/dev/null 2>"$tmpdir/nav-fail-stderr"
code_nav_fail=$?
set -e
...
set +e
(cd "$tmpdir" && PATH="$tmpdir/bin:$PATH" UI_ITERATE_NODE_SCENARIO=timeout UI_ITERATE_CURL_SCENARIO=ok ./scripts/shell/ui-iterate.sh 99 round-10 --route /kiosk --skip-checks) >/dev/null 2>"$tmpdir/timeout-stderr"
code_timeout=$?
set -e
...
rm -f "$node_marker"
set +e
(cd "$tmpdir" && PATH="$tmpdir/bin:$PATH" UI_ITERATE_NODE_MARKER="$node_marker" UI_ITERATE_NODE_SCENARIO=ok UI_ITERATE_CURL_SCENARIO=unreachable ./scripts/shell/ui-iterate.sh 99 round-11 --route /kiosk --skip-checks) >/dev/null 2>"$tmpdir/unreachable-stderr"
code_unreachable=$?
set -e
