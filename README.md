...
Path policy (正本/互換): examples in this README use the canonical path `./scripts/shell/...`.
The `./scripts/agentic-sdd/shell/...` path is retained as a backward-compatible wrapper and is mentioned only when relevant.
...
./scripts/shell/worktree.sh check --issue 123 --issue 124

# Create a worktree per Issue
./scripts/shell/worktree.sh new --issue 123 --desc "add user profile" --tool opencode
./scripts/shell/worktree.sh new --issue 124 --desc "add settings page" --tool opencode
```
...
```bash
# In the target project (after install):
./scripts/shell/ui-iterate.sh 99 --route /kiosk \
...
# In the target project (after install):
# 1) Sync
./scripts/shell/sync-agent-config.sh opencode
...
# In the target project (after install):
# 1) Sync
./scripts/shell/sync-agent-config.sh codex
...
```bash
# In the target project (after install):
./scripts/shell/sync-agent-config.sh all

# Preview (no changes)
./scripts/shell/sync-agent-config.sh --dry-run
```
...
- Enable: `./scripts/shell/setup-githooks.sh` (the installer attempts to configure this automatically)
- Claude Code: `.claude/settings.json` (PreToolUse hooks: Edit/Write + git commit/push)
- OpenCode: `.opencode/plugins/agentic-sdd-gate.js` (generated by `./scripts/shell/sync-agent-config.sh opencode`)

Gate scripts:

- `scripts/gates/validate_worktree.py` (or wrapper: `scripts/validate-worktree.py`)
- `scripts/gates/validate_approval.py` (or wrapper: `scripts/validate-approval.py`)
