# 日時フォーマットルール

ドキュメントやコード内での日時表記の統一ルール。

---

## 基本フォーマット

### 日付

```
YYYY-MM-DD
```

例: `2024-03-15`

### 日時（タイムスタンプ）

```
YYYY-MM-DDTHH:mm:ssZ        # UTC
YYYY-MM-DDTHH:mm:ss+09:00   # JST
```

例: 
- `2024-03-15T10:30:00Z`（UTC）
- `2024-03-15T19:30:00+09:00`（JST）

---

## 使用場面

| 場面 | フォーマット | 例 |
|-----|------------|---|
| ドキュメントの日付 | `YYYY-MM-DD` | `2024-03-15` |
| コミットメッセージ | 不要（gitが自動付与） | - |
| ログ出力 | ISO 8601（UTC推奨） | `2024-03-15T10:30:00Z` |
| APIレスポンス | ISO 8601（UTC推奨） | `2024-03-15T10:30:00Z` |
| ファイル名 | `YYYYMMDD` or `YYYY-MM-DD` | `20240315` or `2024-03-15` |

---

## タイムゾーン

### 原則

- **UTC を基準とする**
- ローカルタイムを使用する場合は、タイムゾーンを明記

### 表記

| 表記 | 意味 |
|-----|------|
| `Z` | UTC（+00:00） |
| `+09:00` | JST（日本標準時） |
| `+00:00` | UTC（明示的） |

---

## ドキュメント内での使用

### PRD / Epic / Issue

```markdown
## メタ情報

| 項目 | 値 |
|-----|---|
| 作成日 | 2024-03-15 |
| 更新日 | 2024-03-20 |
```

### 変更履歴

```markdown
## 変更履歴

| 日付 | バージョン | 変更内容 |
|-----|----------|---------|
| 2024-03-15 | 1.0 | 初版作成 |
| 2024-03-20 | 1.1 | AC追加 |
```

---

## コード内での使用

### 推奨

```typescript
// ISO 8601 形式（UTC）
const timestamp = new Date().toISOString();
// => "2024-03-15T10:30:00.000Z"
```

### 非推奨

```typescript
// ローカルタイム（タイムゾーン不明）
const timestamp = new Date().toString();
// => "Fri Mar 15 2024 19:30:00 GMT+0900 (Japan Standard Time)"
```

---

## 禁止フォーマット

| 禁止 | 理由 | 代替 |
|-----|------|------|
| `03/15/2024` | 曖昧（MM/DD vs DD/MM） | `2024-03-15` |
| `15-Mar-2024` | パース困難 | `2024-03-15` |
| `2024年3月15日` | 日本語依存 | `2024-03-15` |
| タイムゾーンなしの時刻 | 曖昧 | `Z` or `+09:00` を付与 |

---

## 関連ファイル

- `.agent/rules/commit.md` - コミットメッセージルール
- `docs/prd/_template.md` - PRDテンプレート
