...
- Execution policy: autonomous-by-default
  - After the approval record exists, continue `/impl` or `/tdd` -> `/test-review` -> `/review-cycle` -> `/final-review` -> `/test-review` (range on committed `HEAD`) -> `/create-pr` without step-by-step user prompts.
