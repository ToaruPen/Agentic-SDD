...
worktree_sh_src="$repo_root/scripts/shell/worktree.sh"
extractor_src="$repo_root/scripts/extract/extract_issue_files.py"
...
mkdir -p "$tmpdir/scripts/shell" "$tmpdir/scripts/extract"
cp -p "$worktree_sh_src" "$tmpdir/scripts/shell/worktree.sh"
cp -p "$extractor_src" "$tmpdir/scripts/extract/extract_issue_files.py"
cp -rp "$repo_root/scripts/_lib" "$tmpdir/scripts/"
chmod +x "$tmpdir/scripts/shell/worktree.sh"
...
cat >"$tmpdir/scripts/shell/sync-agent-config.sh" <<'EOF'
#!/usr/bin/env bash
set -euo pipefail
printf '%s\n' "$*" >> .sync-agent-config.log
EOF
chmod +x "$tmpdir/scripts/shell/sync-agent-config.sh"
git -C "$tmpdir" add scripts/shell/sync-agent-config.sh
git -C "$tmpdir" -c user.name=test -c user.email=test@example.com commit -m "add sync stub" -q

# Extractor: local file
out1="$(python3 "$tmpdir/scripts/extract/extract_issue_files.py" --repo-root "$tmpdir" --issue-body-file "$tmpdir/issue-1.md" --mode section)"
...
out1_json="$(python3 "$tmpdir/scripts/extract/extract_issue_files.py" --repo-root "$tmpdir" --issue-body-file "$tmpdir/issue-1.json" --mode section)"
...
out_gh="$(PATH="$tmpdir/bin:$PATH" python3 "$tmpdir/scripts/extract/extract_issue_files.py" --repo-root "$tmpdir" --issue 1 --mode section)"
...
set +e
(cd "$tmpdir" && PATH="$tmpdir/bin:$PATH" ./scripts/shell/worktree.sh check --issue 1 --issue 2) >/dev/null 2>"$tmpdir/stderr-gh"
code_gh=$?
set -e
...
(cd "$tmpdir" && PATH="$tmpdir/bin:$PATH" ./scripts/shell/worktree.sh check --issue 1 --issue 3) >/dev/null

(cd "$tmpdir" && ./scripts/shell/worktree.sh bootstrap --tool opencode) >/dev/null
...
(cd "$tmpdir" && ./scripts/shell/worktree.sh remove --dir "$wt_dir")

# Creating the same Issue again should fail unless --use-existing-branch is set
set +e
(cd "$tmpdir" && PATH="$tmpdir/bin:$PATH" ./scripts/shell/worktree.sh new --issue 99 --desc "parallel test" --base HEAD --tool none --worktrees-root "$worktrees_root") >/dev/null 2>"$tmpdir/stderr-issue-lock"
code_lock=$?
set -e
...
wt_dir2="$(cd "$tmpdir" && PATH="$tmpdir/bin:$PATH" ./scripts/shell/worktree.sh new --issue 99 --use-existing-branch --tool none --worktrees-root "$worktrees_root")"
...
(cd "$tmpdir" && ./scripts/shell/worktree.sh remove --dir "$wt_dir2")

wt_dir3="$(cd "$tmpdir" && PATH="$tmpdir/bin:$PATH" ./scripts/shell/worktree.sh new --issue 100 --desc "sync fallback" --base HEAD --tool opencode --worktrees-root "$worktrees_root")"
...
(cd "$tmpdir" && ./scripts/shell/worktree.sh remove --dir "$wt_dir3")
