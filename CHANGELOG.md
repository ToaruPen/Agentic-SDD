...
- Change `/review-cycle` default Codex model to `gpt-5.3-codex`.
- Keep `/review-cycle` default `REASONING_EFFORT` as `high`.
- Update OpenCode reviewer agent (`sdd-reviewer`) to `openai/gpt-5.3-codex` (keep `reasoningEffort: high`).
