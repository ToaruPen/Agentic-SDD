...
cleanup_sh_src="$repo_root/scripts/shell/cleanup.sh"
...
cp -p "$cleanup_sh_src" "$tmpdir/scripts/shell/cleanup.sh"
chmod +x "$tmpdir/scripts/shell/cleanup.sh"
...
# dry-run は削除しない
(cd "$tmpdir" && ./scripts/shell/cleanup.sh 123 --dry-run) >/dev/null
if ! git -C "$tmpdir" show-ref --verify --quiet "refs/heads/$branch1"; then
	eprint "dry-run unexpectedly deleted branch: $branch1"
	exit 1
fi

(cd "$tmpdir" && ./scripts/shell/cleanup.sh 123) >/dev/null
if git -C "$tmpdir" show-ref --verify --quiet "refs/heads/$branch1"; then
...
(cd "$tmpdir" && ./scripts/shell/cleanup.sh --all) >/dev/null
